...
def test_and_train_run(NB_EPOCH):
...
for i in range(0,M):
	print("------------------------------------------------------------------")
	print("NB_EPOCH:",NB_EPOCH[i])
	val_score[i],val_accuracy[i] = test_and_train_run(NB_EPOCH[i])
	print("------------------------------------------------------------------")

# View the Test accuracy to different hyperparameter values
plt.plot(NB_EPOCH,val_accuracy)
plt.title('NB_EPOCH tuning')
plt.ylabel('Accuracy -->')
plt.xlabel('epoch -->')
